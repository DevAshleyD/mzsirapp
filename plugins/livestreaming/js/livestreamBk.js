...
function arrayFunc(xs, yx, d){
	if(xs.sort().toString() == yx.sort().toString()) return 1;
	let empt = x => y => x.length == 0 ? y.length != 0 ? y : 0 : y.length != 0 ? 1 : x; 
	const a_empty = empt(xs)(yx);
	if(a_empty instanceof Array) return a_empty;
	else if(a_empty == 0) return [];
	
	//If the program has not returned we got works to do let's go
	...
	if(d == 'left')return differencel(xs)(ys);
	//right difference
	if(d == 'join') return flip(differencel)(xs)(ys);
	
	//union
	if(d == "union"){
		return [...xs, ...yx];
	}
	// symmetric difference: change
...
function refreshData(data){
	...
	if(data._join){
		$.notify({
			icon: data._join.icon,
			title: data._join.title,
			message: data._join.message
		},{
			type: 'minimalist',
			delay: 5000,
			icon_type: 'image',
	...
	if(data._left){
		$.notify({
			icon: data._left.icon,
			title: data._left.title,
			message: data._left.message
		},{
			type: 'minimalist',
			delay: 5000,
			icon_type: 'image',
